...
export default async function NewSalePage({
  ...
  const initialType = params.type === "membership" ? "membership" : "package";
  ...
  const membershipAllowed = canSellMemberships(context.studioRole, context.isPlatformAdmin);

  if (!canSellPackages && !membershipAllowed) {
    redirect("/app");
  }

  const studioId = context.studioId;

  const [packageTemplatesResult, clientsResult, ledgerResult, plansResult, existingResult] = await Promise.all([
    supabase
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-7 text-white/85 md:text-base">
                Choose a client, choose what they are buying, and complete package or membership sales from one guided front-desk workflow.
              </p>
            </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-3xl font-semibold text-slate-950">Unified</p>
            </div>
            <div className="rounded-2xl bg-[var(--brand-primary-soft)] p-3 text-[var(--brand-primary)]">
