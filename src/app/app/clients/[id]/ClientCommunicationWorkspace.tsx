...
export default function ClientCommunicationWorkspace({
  ...
  lifecycleRiskReason: string | null;
  lifecycleActionLabel: string;
  lifecycleActionHref: string | null;
  lifecycleAriaPrompt: string;
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[28px] border border-violet-200/80 bg-[linear-gradient(135deg,#111827_0%,#4c1d95_58%,#f97316_150%)] p-5 text-white shadow-[0_18px_45px_rgba(76,29,149,0.12)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/70">
              Journey-guided communication
            </p>
            <h3 className="mt-2 text-xl font-semibold">{lifecycleLabel}</h3>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/85">
              {lifecycleNextStep}
            </p>
            {lifecycleRiskReason ? (
              <p className="mt-2 text-sm font-medium text-orange-100">
                Risk signal: {lifecycleRiskReason}
              </p>
            ) : null}
            <p className="mt-2 text-xs font-semibold text-violet-100">
              ARIA: {lifecycleAriaPrompt}
            </p>
          </div>
          {lifecycleActionHref ? (
            <Link
              href={lifecycleActionHref}
              className="inline-flex shrink-0 items-center justify-center rounded-xl border border-white/20 bg-white px-4 py-2.5 text-sm font-semibold text-violet-900 shadow-sm hover:bg-white/90"
            >
              {lifecycleActionLabel}
            </Link>
          ) : null}
