...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (clientError) return new NextResponse(`Failed to load client: ${clientError.message}`, { status: 500 });
  ...
  if (!client || !signature) return new NextResponse("Signed document not found", { status: 404 });
    ...
    sections: [
      {
        ...
        rows: [
          ["Client", clientName],
          ["Client Email", typedClient.email ?? ""],
          ["Client Phone", typedClient.phone ?? ""],
          ["Client ID", typedClient.id],
        ],
      },
      {
        ...
        rows: [
          ...
          ["Signer Email", typedSignature.signer_email ?? typedClient.email ?? ""],
  ...
  const filename = `danceflow-signed-document-${slugifyFilename(title)}-${slugifyFilename(clientName)}.pdf`;
