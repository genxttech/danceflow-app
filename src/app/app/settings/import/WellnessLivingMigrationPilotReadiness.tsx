...
const STAGES: StageDefinition[] = [
  {
    key: "clients",
    label: "Clients",
    description: "Identity and contact records",
    required: true,
  },
  {
    key: "instructors",
    label: "Instructors",
    description: "Teaching staff and source identity",
    required: true,
  },
  {
    key: "packages",
    label: "Packages",
    description: "Pricing options and remaining visits",
    required: false,
  },
  {
    key: "memberships",
    label: "Memberships",
    description: "Plans, current periods, and payment state",
    required: false,
  },
  {
    key: "appointments",
    label: "Appointments",
    description: "Schedule history and future bookings",
    required: true,
  },
  {
    key: "attendance",
    label: "Attendance",
    description: "Historical attendance without repeat deductions",
    required: false,
  },
  {
    key: "payments",
    label: "Payments",
    description: "Historical financial activity",
    required: true,
  },
  {
    key: "account_credits",
    label: "Account Credits",
    description: "Credit and debit ledger reconciliation",
    required: false,
  },
];
...
export default async function WellnessLivingMigrationPilotReadiness({
  supabase,
  studioId,
}: Props) {
    ...
    return (
      <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-amber-700">
          WellnessLiving Pilot Readiness
        </p>
        <p className="mt-2 text-sm text-amber-800">
          Readiness data could not be loaded. Refresh after confirming the import
  ...
  const requiredReady = requiredStages.filter(
    (row) => row.status === "ready",
  ).length;
  const attentionCount = stageResults.filter(
  ...
  const relationshipReviewRows = batches.reduce(
    (total, batch) =>
      total + readNumber(batch.summary, "relationship_review_rows"),
    0,
  );
  const futureBillingSetupRequired = batches.reduce(
    (total, batch) =>
      total + readNumber(batch.summary, "future_billing_setup_required"),
    0,
  );
  const currentPackageBalanceUnits = batches.reduce(
    (total, batch) =>
      total + readNumber(batch.summary, "current_balance_units"),
    0,
  );
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-white/75">
              WellnessLiving Migration
            </p>
            <h2 className="mt-2 text-2xl font-semibold">
              Pilot readiness and reconciliation
            </h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/85">
              Confirm that identity, schedule, financial history, balances, and
              remaining exceptions are reconciled before studio activation.
            </p>
          </div>
              ...
              <div className="mt-4 text-xs text-[#6F5A7A]">
                {batch ? (
                  <>
                    ...
                    <p className="mt-1">
                      Reconciliation:{" "}
                      {batch.reconciliation_status.replaceAll("_", " ")}
                    </p>
                  </>
                ) : (
                  <p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4">
            <p className="text-sm font-semibold text-violet-800">
              Package balances preserved
            </p>
            <p className="mt-2 text-2xl font-semibold text-violet-900">
              {currentPackageBalanceUnits}
            </p>
            <p className="mt-1 text-xs text-violet-700">
              Remaining usage units imported without historical re-deduction.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4">
            ...
            <p className="mt-1 text-xs text-orange-700">
              Credits minus debits from executed WellnessLiving ledger batches.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-900">
              Do not activate the migrated studio until required stages are
              reconciled, failed rows are resolved, and future billing has been
