...
export default async function PublicEventDetailPage({
  ...
  const [
    ...
    supabase.from("event_tags").select("id, tag").eq("event_id", typedEvent.id).order("tag", { ascending: true }),
    supabase
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white shadow-sm">
        <div className="relative">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
            <div className="flex flex-wrap gap-2">
              <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${eventTypeBadgeClass(typedEvent.event_type)}`}>
                {eventTypeLabel(typedEvent.event_type)}
              </span>

              {typedEvent.featured ? (
                <span className="inline-flex rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-900">
                  Featured
                </span>
              ) : null}
            ...
            <p className="mt-3 max-w-3xl text-base leading-7 text-slate-100 sm:text-lg">
              {typedEvent.short_description || "Event details are listed below."}
            </p>

            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="#registration"
                className="inline-flex rounded-xl bg-white px-4 py-2 text-sm font-medium text-slate-900 transition hover:bg-slate-100"
              >
                Register Now
              </Link>
              <Link
                href="/discover/events"
                className="inline-flex rounded-xl border border-white/30 bg-white/10 px-4 py-2 text-sm font-medium text-white transition hover:bg-white/20"
              >
                Back to Events
              </Link>
            </div>
        ...
        <div className="grid gap-4 border-t border-slate-200/80 bg-slate-50/80 p-6 sm:grid-cols-2 xl:grid-cols-4">
          <InfoCard
            label="Dates"
          ...
          <InfoCard
            label="Location"
            value={[typedEvent.city, typedEvent.state].filter(Boolean).join(", ") || "Location TBD"}
            detail={typedEvent.venue_name || undefined}
          />
          <InfoCard
            label="Organizer"
            value={organizer?.name ?? "Unknown organizer"}
          />
          <InfoCard
            label="Availability"
            value={
              typedEvent.capacity == null
                ? "Open registration"
                : `${eventRemainingCapacity ?? 0} spots left`
            }
            detail={typedEvent.waitlist_enabled ? "Waitlist available when full" : undefined}
          />
        </div>
        ...
        <div className="space-y-6">
          <section className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm sm:p-8">
            <h2 className="text-2xl font-semibold tracking-tight text-slate-950">
              About This {typedEvent.event_type === "group_class" ? "Class" : "Event"}
            </h2>
            <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-700">
              {typedEvent.description || "No full description has been added yet."}
            </p>
          </section>

          <section className="rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm sm:p-8">
            <h2 className="text-2xl font-semibold tracking-tight text-slate-950">Venue & Details</h2>

            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <InfoCard label="Venue" value={typedEvent.venue_name || "Venue TBD"} />
              <InfoCard label="Timezone" value={typedEvent.timezone} />
              <div className="sm:col-span-2 rounded-2xl border bg-slate-50 p-4">
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <p className="mt-4 text-xs leading-5 text-slate-500">
              Free registrations complete immediately. Paid tickets continue to secure Stripe Checkout.
              If the event is full and waitlist is enabled, you can join the waitlist without being charged.
            </p>
          </section>
            ...
            <div className="mt-5 space-y-4">
              ...
              {organizer?.contact_email ? (
                <div className="rounded-xl border bg-slate-50 p-4">
                  ...
                  <p className="mt-1 font-medium text-slate-900">{organizer.contact_email}</p>
                </div>
              ) : null}
