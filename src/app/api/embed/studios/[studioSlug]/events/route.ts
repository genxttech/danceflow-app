...
export async function GET(
  ...
  const normalizedSlug = studioSlug.trim().toLowerCase();

  if (!SLUG_PATTERN.test(normalizedSlug)) {
    return NextResponse.json(
      { error: "Studio event embed not found." },
  ...
  if (studioError) {
    return NextResponse.json(
      { error: "Could not load studio events." },
  ...
  const hostName = studio.public_name || studio.name || "DanceFlow Studio";
