...
export async function GET(
  ...
  const { data: envelope, error: envelopeError } = await admin
    ...
    .select(
      "id, studio_id, client_id, assignment_id, status, signed_bucket, signed_path",
    )
  ...
  if (envelope.status !== "completed") {
    return studentApiJsonError("The signed document is not ready.", 409);
  }

  const bucket = envelope.signed_bucket;
  const path = envelope.signed_path;

  if (!bucket || !path) {
    return studentApiJsonError("The signed PDF is unavailable.", 404);
  }
  ...
  return new Response(bytes, {
    ...
    headers: {
      ...
      "Content-Disposition": 'inline; filename="danceflow-signed-document.pdf"',
