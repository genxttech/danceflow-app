...
export default function AnalyticsWorkspaceNav({ activeSection = "overview", range }: { activeSection?: AnalyticsSection; range?: string }) {
  return (
    <nav aria-label="Analytics sections" className="overflow-x-auto rounded-[24px] border border-violet-200/70 bg-white p-2 shadow-sm">
      <div className="flex min-w-max gap-1">
        {ITEMS.map((item) => {
          ...
          return (
            <Link key={item.id} href={href} aria-current={active ? "page" : undefined} className={["inline-flex items-center gap-2 rounded-2xl px-3.5 py-2.5 text-sm font-semibold transition", active ? "bg-[linear-gradient(135deg,#ede9fe_0%,#ffedd5_100%)] text-violet-950 ring-1 ring-violet-200" : "text-slate-600 hover:bg-slate-50 hover:text-violet-900"].join(" ")}>
