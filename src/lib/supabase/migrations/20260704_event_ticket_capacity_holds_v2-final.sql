...
begin
  ...
  requested_quantity := greatest(coalesce(new.quantity, 1), 1);
  ...
  select coalesce(sum(greatest(coalesce(er.quantity, 1), 1)), 0)
  ...
  if reserved_quantity + requested_quantity > ticket_capacity then
    raise exception
      'Selected ticket quantity is no longer available. % ticket(s) remain.',
      greatest(ticket_capacity - reserved_quantity, 0)
      using errcode = 'P0001';
  end if;
