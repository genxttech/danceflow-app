...
function safeCsv(headers: string[], rows: Array<Array<unknown>>) {
  return toCsv(
    headers.map((header) => String(safeCsvCell(header))),
...
export async function buildAccountantReport(params: { studioId: string; reportType: AccountantReportType; range: string }) {
  ...
  if (params.reportType === "accounting_ledger" || params.reportType === "profit_loss") {
    ...
    if (params.reportType === "accounting_ledger") {
      return { filename: `danceflow-accounting-ledger-${params.range}.csv`, csv: safeCsv([
    ...
    return { filename: `danceflow-profit-loss-${params.range}.csv`, csv: safeCsv(["Metric","Amount"], [["Gross Revenue",totals.revenue],["Refunds",totals.refunds],["Fees",totals.fees],["Expenses",totals.expenses],["Estimated Net",net]]) };
  }
  ...
  if (params.reportType === "payments_refunds") {
    ...
    return { filename:`danceflow-payments-refunds-${params.range}.csv`, csv:safeCsv(["Payment ID","Amount","Refunded Amount","Payment Method","Status","Created At","Notes"], (data??[]).map((r) => [r.id,r.amount,Math.max(Number(r.refunded_amount??0),Number(r.refund_amount??0)),r.payment_method,r.status,r.created_at,r.notes])) };
  }
  ...
  if (params.reportType === "expenses") {
    ...
    if (error) throw new Error(error.message);
    return { filename:`danceflow-expenses-${params.range}.csv`, csv:safeCsv(["Expense ID","Expense Date","Vendor","Category","Amount","Currency","Payment Method","Notes","Created At"], (data??[]).map((r) => [r.id,r.expense_date,r.vendor_name,r.category,r.amount,r.currency,r.payment_method,r.notes,r.created_at])) };
  }
  ...
  return { filename:`danceflow-event-profitability-${params.range}.csv`, csv:safeCsv(["Event ID","Gross Revenue","Refunds","Fees","Net Ticket Revenue","Event Expenses","Labor Costs","Profit Loss"], Array.from(map.entries()).map(([id,v])=>[id,v.gross,v.refunds,v.fees,v.net,v.expenses,v.labor,v.net-v.expenses-v.labor])) };
}
